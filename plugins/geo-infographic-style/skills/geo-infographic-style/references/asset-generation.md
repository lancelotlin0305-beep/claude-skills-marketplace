...
| 需求 | 路徑 | 工具 | 去背方式 |
|---|---|---|---|
| **單一物件**(每卡 1 件、透明浮貼) | 單物件去背 | `gen_assets.js` | 鋪 chroma 底(預設洋紅)→ `chroma_key.js` 自動偵測挖除 → 透明 PNG |
| **一整組呼應文字的場景**(多元件、玻璃/科技風) | 場景式(進階) | `gen_scene.js`(參考圖鎖風格) | 不透明霧面 + chroma 底 → 去背浮貼(見「場景式素材」) |
| **需真半透明玻璃** | OpenAI 真透明 | `gen_assets.js`(`PROVIDER=openai`) | `background=transparent`(真 alpha,免挖) |
| **雲端/受限環境、無法本機外呼** | Prompt 交接(離線) | 使用者自產 → 貼回 | 收圖後同上,`chroma_key.js` 去背(見「Prompt 交接模式」) |

**去背後嵌入時素材已自帶光影,不再疊 SVG 物件投影與地面橢圓。** 選定路徑後,單物件/交接走下方決策流程,場景式走「場景式素材」節。

> **素材可搶眼(與文字區的淺色原則分開)**:承載文字的表面維持淺色(可讀性);但**素材/插圖本身不受淺色限制**——可深色、飽和、發光,以抓眼球為目標(prompt 色系照卡色指定即可,不必刻意調淡)。詳見 style-spec §2「表面分類」。
...
當執行環境**無法本機外呼 API**(如 Claude.ai 網頁、受限沙箱、無 Node/金鑰),仍要走第 3 級素材時,改由**使用者代為產圖**,Claude 只負責出 prompt 與收圖後的組裝:
...
> 交接模式產出物與自動模式**完全等價**——差別只在「產圖那一步由使用者代勞」;去背、驗收、組裝、留存規則不變。金鑰安全鐵則在此模式下自然滿足(Claude 不碰任何金鑰)。
