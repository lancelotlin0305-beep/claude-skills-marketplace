// [工具] 多模態場景式素材:風格參考圖 + 內容 prompt → 一整組會呼應文字的等角場景(玻璃風格免洋紅去背)。
...
(async()=>{
  ...
  for(let i=0;i<6;i++){
    ...
    if(r.status===200){
      ...
      if(img){fs.writeFileSync(OUT,Buffer.from((img.inlineData||img.inline_data).data,'base64'));console.log('OK',OUT);return;}
