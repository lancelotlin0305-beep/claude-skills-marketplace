...
P(`<text x="${tipX-6}" y="${tipY-16}" font-size="16" font-weight="800" fill="${ACC}" text-anchor="end">願景</text>`);
...
cards.forEach((cd,i)=>{
  ...
  P(`<rect x="${x+pad}" y="${cy+18}" width="${pw}" height="30" rx="15" fill="url(#${R.hdr})" filter="url(#row)"/>`);
  P(`<text x="${x+pad+pw/2}" y="${cy+38}" font-size="16" font-weight="700" fill="#fff" text-anchor="middle">${esc(cd.pill)}</text>`);
  P(`<text x="${x+pad}" y="${cy+78}" font-size="21" font-weight="800" fill="${R.deep}">${esc(cd.title)}</text>`);
  P(`<line x1="${x+pad}" y1="${cy+90}" x2="${x+cw-pad}" y2="${cy+90}" stroke="${R.edge}" stroke-width="1.5"/>`);
  const blh=27;
  cd.body.forEach((ln,j)=>{P(`<text x="${x+pad}" y="${cy+116+j*blh}" font-size="14.5" font-weight="500" fill="${INK}">${esc(ln)}</text>`);});
  const sTop=cy+100+cd.body.length*blh+6, sBot=cy+ch-8;
...
const cxs=8,cgap=12,ciw=(bw-48-(cxs-1)*cgap)/cxs,cyy=by+80,cih=104;
