...
async function genGemini(prompt){
  const body=JSON.stringify({contents:[{parts:[{text:prompt+CHROMA}]}],generationConfig:{responseModalities:["Image"]}});
...
(async()=>{
  ...
  for(const it of items){
    ...
    saved.push({id:it.id,name:it.name,file,provider:PROVIDER,model,prompt:basePrompt});
    fs.writeFileSync(manifestOut,JSON.stringify(saved,null,2));
    await sleep(3000);
  }
  console.log('prompts 已存:',manifestOut);
  if(PROVIDER!=='openai') console.log('下一步:跑 chroma_key.js 去背(洋紅底)');
