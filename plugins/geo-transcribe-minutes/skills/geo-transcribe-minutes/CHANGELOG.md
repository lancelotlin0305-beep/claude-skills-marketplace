...
- **輸入面**:既有逐字稿(txt/md/srt)重新開放為本 skill 會議模式入口(免轉錄直接
  ...
  歷次會議記錄(啟用輕量版「前次待辦進度回顧」,狀態含「未提及」誠實標記,
  待辦編號延續)。
- **輸出面**:會議模式定為「標準交付組合」四件——`<會議名>_逐字稿.md`、
  `<會議名>_重點摘要.md`、`<會議名>_重點摘要.html`(SVG 視覺版)、
  `YYYYMMDD_<會議名>_會議紀錄_正式版_v1.docx`(公文體,呼叫 minutes-builder 的
  build_official_minutes.py;workflow 新增 §5b 對映速查)。
- 分工調整:minutes-builder 深度 lane 收斂為知識萃取、對外/內部完整章節、
  **嚴格**跨會議追蹤(懸置累計/逾期分析/差異對照)、模式 B。
