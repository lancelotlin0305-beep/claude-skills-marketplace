...
## 產出(輸出資料夾 `YYYYMMDD-<相關名稱>\`;檔名一律 `YYYYMMDD-<相關名稱>` 前綴,離開資料夾仍自明)

**檔案輸出慣例(鐵則)**:每次產檔一律**先建一個資料夾**把該批相關檔案(MD/HTML/DOCX/
截圖等)全放進去;資料夾名與**每個**檔名都以**產出當天絕對日期** `YYYYMMDD-` 為前綴
(例:資料夾 `20260820-XX會議記錄\`,內含 `20260820-XX_逐字稿.md`、
`20260820-XX_重點摘要.html`)。使用者另有指定則從之。

**重點摘要截取關鍵畫面(影音來源)**:產重點摘要時,若來源有重要影音畫面——**操作介面、
圖文簡報頁、demo 操作、關鍵數據圖表**——一律截圖存入該批資料夾(子夾 `YYYYMMDD-截圖\`),
並以 base64 內嵌進單檔自足 HTML,截圖標時間戳(近似值時註記)。作法見 workflow §2b。
